...
class AdminHelpHandler {
    ...
    static async callTicketOwner(interaction) {
        const ticket = ticketsDB.Tickets_Support.find(t => t.Ticket === interaction.channel.id);
        if (!ticket) {
...
client.on('interactionCreate', async (interaction) => {
    ...
    try {
        ...
        if (interaction.customId === 'AdminHelpMenu') {
            ...
            switch (value) {
                case 'call_owner':
                    await AdminHelpHandler.callTicketOwner(interaction);
                    break;
                case 'add_user':
                case 'remove_user':
                    await AdminHelpHandler.showUserModal(interaction, value);
                    break;
                case 'rename_ticket':
