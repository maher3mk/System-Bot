...
class MzadHelpHandler {
    ...
    static async callTicketOwner(interaction) {
        const ticketmzad = ticketsDB.Tickets_Mzad.find(t => t.Ticket === interaction.channel.id);
        if (!ticketmzad) {
            return interaction.reply({ content: '❌ لم يتم العثور على بيانات التذكرة.', ephemeral: true });
        }

        const owner = ticketmzad.userid;
...
client.on('interactionCreate', async (interaction) => {
    ...
    try {
        if (interaction.customId === 'MzadHelp') {
            await MzadHelpHandler.showMainMenu(interaction);
        }

        if (interaction.customId === 'MzadHelpMenu') {
            const value = interaction.values[0];
            
            switch (value) {
                case 'call_owner':
                    await MzadHelpHandler.callTicketOwner(interaction);
                    break;
                case 'add_user':
                case 'remove_user':
                    await MzadHelpHandler.showUserModal(interaction, value);
                    break;
                case 'rename_ticket':
                    await MzadHelpHandler.showRenameModal(interaction);
                    break;
                case 'call_leader':
                    await MzadHelpHandler.showLeaderModal(interaction);
                    break;
                case 'reset_menu':
                    await MzadHelpHandler.showMainMenu(interaction);
                    break;
        ...
        if (interaction.isModalSubmit()) {
            switch (interaction.customId) {
                ...
                case 'RemoveUserModal':
                    await MzadHelpHandler.handleUserModal(interaction);
                    break;
                case 'RenameTicket':
                    await MzadHelpHandler.handleRename(interaction);
                    break;
                case 'CallLeaderModal':
                    await MzadHelpHandler.callLeader(interaction);
                    break;
            }
        }
    } catch (error) {
