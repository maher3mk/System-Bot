...
class SpinHelpHandler {
    ...
    static async callTicketOwner(interaction) {
        const ticketspin = ticketsDB.Tickets_Spin.find(t => t.Ticket === interaction.channel.id);
        if (!ticketspin) {
            return interaction.reply({ content: '❌ لم يتم العثور على بيانات التذكرة.', ephemeral: true });
        }

        const owner = ticketspin.userid;
...
client.on('interactionCreate', async (interaction) => {
    ...
    try {
        if (interaction.customId === 'SpinHelp') {
            await SpinHelpHandler.showMainMenu(interaction);
        }

        if (interaction.customId === 'SpinHelpMenu') {
            const value = interaction.values[0];
            
            switch (value) {
                case 'call_owner':
                    await SpinHelpHandler.callTicketOwner(interaction);
                    break;
                case 'add_user':
                case 'remove_user':
                    await SpinHelpHandler.showUserModal(interaction, value);
                    break;
                case 'rename_ticket':
                    await SpinHelpHandler.showRenameModal(interaction);
                    break;
                case 'call_leader':
                    await SpinHelpHandler.showLeaderModal(interaction);
                    break;
                case 'reset_menu':
                    await SpinHelpHandler.showMainMenu(interaction);
                    break;
        ...
        if (interaction.isModalSubmit()) {
            switch (interaction.customId) {
                ...
                case 'RemoveUserModal':
                    await SpinHelpHandler.handleUserModal(interaction);
                    break;
                case 'RenameTicket':
                    await SpinHelpHandler.handleRename(interaction);
                    break;
                case 'CallLeaderModal':
                    await SpinHelpHandler.callLeader(interaction);
                    break;
            }
        }
    } catch (error) {
